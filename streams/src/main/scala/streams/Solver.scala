...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    for ((block, listMove) <- neighbors if !explored.contains(block)) yield (block, listMove)
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, listMove) #:: tail => {
      val exploredNew = explored + block
      val neighborsNew = newNeighborsOnly(neighborsWithHistory(block, listMove), exploredNew)
